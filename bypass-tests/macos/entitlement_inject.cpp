...
int main(void) {
    std::printf("DISABLED: entitlement_inject.cpp activates once the Phase-5 signed-fixture + "
...
int main(void) {
    /* Phase 5 fills this in: re-sign adding get-task-allow / disable-library-validation; assert HK_CS_ENTITLEMENT_DRIFT fires while a Rosetta OS-injected entitlement does NOT (allowlist proof), then query the daemon / ac_get_last_flag and
     * assert the expected HK_CS_* finding fires (and that the FP-gate case does
     * NOT). */
    std::printf("entitlement_inject.cpp: enforcement path not yet implemented.\n");
    return 1;
}

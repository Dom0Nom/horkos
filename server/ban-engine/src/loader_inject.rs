...
mod tests {
    ...
    #[test]
    fn empty_evidence_never_bans() {
        let e = InjectionEvidence::default();
        assert_eq!(e.score(), 0);
        assert!(!e.proposes_ban());
    }

    #[test]
    fn loadorder_inversion_alone_never_reaches_threshold() {
        // Signal 87 standalone — the catalog's hardest FP rule. Must NOT ban.
        let e = InjectionEvidence {
            loadorder_invert: true,
            ..Default::default()
        };
        assert_eq!(e.score(), 0); // corroborating-only: not counted standalone
        assert!(!e.proposes_ban());
    }

    #[test]
    fn loadorder_only_counts_when_corroborated() {
        // 82 (40) alone is below the 70 threshold...
        let dso_only = InjectionEvidence {
            dso_no_provenance: true,
            ..Default::default()
        };
        assert_eq!(dso_only.score(), W_DSO_NO_PROVENANCE);
        assert!(!dso_only.proposes_ban());

        // ...but 82 + 87-corroboration (40 + 10 = 50) is still below threshold
        // (87 only adds its small corroborating weight, never enough alone).
        let dso_plus_order = InjectionEvidence {
            dso_no_provenance: true,
            loadorder_invert: true,
            ..Default::default()
        };
        assert_eq!(
            dso_plus_order.score(),
            W_DSO_NO_PROVENANCE + W_LOADORDER_CORROBORATING
        );
        assert!(!dso_plus_order.proposes_ban());
    }

    #[test]
    fn high_confidence_signal_plus_corroboration_bans() {
        // 83 GOT redirect (60) + 82 (40) = 100 >= 70.
        let e = InjectionEvidence {
            got_redirect: true,
            dso_no_provenance: true,
            ..Default::default()
        };
        assert!(e.score() >= BAN_THRESHOLD);
        assert!(e.proposes_ban());
    }

    #[test]
    fn preload_ancestor_attribution_is_carried() {
        let e = InjectionEvidence {
            preload_transient: true,
            preload_ancestor_pid: 4242,
            ..Default::default()
        };
        assert_eq!(e.preload_ancestor_pid, 4242);
        // 85 alone (45) is below threshold — needs corroboration to ban.
        assert!(!e.proposes_ban());
    }

    #[test]
    fn single_low_fp_signal_does_not_ban_alone() {
        for e in [
            InjectionEvidence {
                interp_mismatch: true,
                ..Default::default()
            },
            InjectionEvidence {
                preload_transient: true,
            ...
            assert!(!e.proposes_ban(), "single signal must not ban alone: {e:?}");
        }
    }
}

...
mod tests {
    ...
    fn obs(signal_id: u16, features: [f32; AIM_FEATURE_DIM]) -> AimObservation {
        AimObservation {
            signal_id,
            features,
    ...
    #[test]
    fn missing_model_env_is_none() {
        // No env var -> fail-open, no scorer, no panic.
        std::env::remove_var("HORKOS_AIM_MODEL");
        assert!(AimScorer::from_env().is_none());
    }
    ...
    #[test]
    fn segmenter_to_model_chain_scores_botlike_flicks() {
        use telemetry::schema::TickPayload;
        // Synthesize many bot-like flicks: each is a sharp angular-velocity
        // peak that closes hard onto the target (overshoot ~0, extreme jerk) —
        // the structural signature segment_flicks extracts and the model scores.
        let mut window = Vec::new();
        let mut ts = 1_000_000u64;
        for _ in 0..16 {
            // approach: rising velocity, distance shrinking
            for (v, d, j) in [
                (0.2f32, 0.30f32, -2.0f32),
                (1.5, 0.10, -40.0),
                (0.1, 0.02, -5.0),
            ] {
                window.push(TickPayload {
                    ang_vel: v,
                    ang_jerk: j,
                    dist_to_nearest_target_rad: d,
                    hid_newest_ts_ns: ts,
                    ..Default::default()
                });
                ts += 2_000_000;
        ...
        let flicks = crate::aim_kinematics::segment_flicks(&window);
        assert!(
            flicks.len() as u32 >= MIN_AIM_EVENTS,
            "got {} flicks",
            flicks.len()
        );
        let obs = observations_from_segments(&flicks, &[], &[], &[], 500);
        let flick_obs = obs
            .iter()
    ...
    #[test]
    fn observations_respect_recurrence_floor() {
        let few = vec![
            FlickFeatures {
                overshoot_ratio: 0.0,
                settle_ms: 1.0,
            ...
            (MIN_AIM_EVENTS - 1) as usize
        ];
        assert!(observations_from_segments(&few, &[], &[], &[], 100).is_empty());
        ...
        let enough = vec![
            ...
            MIN_AIM_EVENTS as usize
        ];
        let obs = observations_from_segments(&enough, &[], &[], &[], 100);
        assert_eq!(obs.len(), 1);
        assert_eq!(obs[0].signal_id, SIGNAL_FLICK_CURVATURE);
        assert_eq!(obs[0].features[2], 9.0); // mean |jerk|
    }
}

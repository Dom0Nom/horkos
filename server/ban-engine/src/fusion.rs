...
mod tests {
    ...
    fn empty_input_is_clean() {
        let out = fuse(&[], None, &FusionParams::default()).expect("fuse");
        assert_eq!(out.verdict, Verdict::Clean);
        assert_eq!(out.score, 0);
    }

    #[test]
    fn single_signal_never_bans_even_at_extreme_z() {
        // The no-single-signal invariant: one signal, astronomic z, huge support
        // -> Review at most.
        let out = fuse(&[ev(172, 1.0e6, u32::MAX)], None, &FusionParams::default()).expect("fuse");
        assert_ne!(out.verdict, Verdict::Ban);
    }

    #[test]
    fn duplicate_signal_id_cannot_self_corroborate() {
        // Two events with the SAME id must not satisfy the distinct-signal gate.
        let out = fuse(
            &[ev(172, 9.0, 50), ev(172, 8.0, 50)],
            None,
    ...
    #[test]
    fn two_strong_distinct_signals_ban() {
        let out = fuse(
            &[ev(172, 5.0, 20), ev(178, 6.5, 64)],
            None,
            &FusionParams::default(),
        )
        .expect("fuse");
        assert_eq!(out.verdict, Verdict::Ban);
        assert!(out.score >= 70);
    }

    #[test]
    fn strong_signal_plus_flagged_cadence_reviews_not_bans() {
        // 40 + 25 = 65 < ban_threshold: corroborated but under-weighted ->
        // Review. A ban needs two STRONG independent signals under the default
        // priors (conservative by construction).
        let obs = CadenceObservation {
            conserved_repeats: 3,
            worst_conservation: 1.02,
            flagged: true,
        };
        let out = fuse(&[ev(172, 5.0, 20)], Some(obs), &FusionParams::default()).expect("fuse");
        assert_eq!(out.verdict, Verdict::Review);
        assert_eq!(out.contributions.len(), 2);
    }

    #[test]
    fn unflagged_cadence_contributes_nothing() {
        let obs = CadenceObservation {
            conserved_repeats: 1,
            worst_conservation: 1.4,
            flagged: false,
        };
        let out = fuse(&[], Some(obs), &FusionParams::default()).expect("fuse");
        assert_eq!(out.verdict, Verdict::Clean);
        assert!(out.contributions.is_empty());
    }

    #[test]
    fn below_tier_signals_are_skipped_not_summed() {
        // Nine sub-threshold signals must not aggregate into a verdict.
        let events: Vec<_> = (172u16..=180).map(|s| ev(s, 2.4, 50)).collect();
        let out = fuse(&events, None, &FusionParams::default()).expect("fuse");
        assert_eq!(out.verdict, Verdict::Clean);
        assert_eq!(out.score, 0);
        assert_eq!(out.skipped.len(), 9);
    }

    #[test]
    fn non_finite_z_is_skipped_not_fatal() {
        // A NaN from one buggy analyzer must not blind the others (fail-open on
        // the remaining evidence, recorded as skipped).
        let out = fuse(
            &[ev(172, f64::NAN, 50), ev(178, 5.0, 64), ev(173, 5.0, 30)],
            None,
            &FusionParams::default(),
        )
        .expect("fuse");
        ...
        assert_eq!(out.skipped[0].reason, "non-finite zscore");
    }

    #[test]
    fn under_sampled_event_is_skipped() {
        let out = fuse(&[ev(172, 9.0, 2)], None, &FusionParams::default()).expect("fuse");
        assert_eq!(out.verdict, Verdict::Clean);
        assert_eq!(out.skipped[0].reason, "below sample floor");
    }

    #[test]
    fn params_rejecting_single_signal_ban_config() {
        let p = FusionParams {
    ...
    #[test]
    fn params_reject_non_finite_tiers() {
        let p = FusionParams {
            z_moderate: f64::NAN,
            ..Default::default()
        };
    ...
    #[test]
    fn verdict_ordering_supports_latching() {
        assert!(Verdict::Clean < Verdict::Review);
        assert!(Verdict::Review < Verdict::Ban);
    }
}

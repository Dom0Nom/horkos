...
mod tests {
    ...
    #[test]
    fn cadence_features_round_trip() {
        let f = CadenceFeatures {
            schema_version: DEVICE_TRUST_SCHEMA_VERSION,
            reserved0: 0,
            hdevice_token: 0xfeed,
            declared_interval_ms: 1.0,
            observed_rate_hz: 2000.0,
            ceiling_violation_ratio: 2.0,
            device_lifetime_s: 5.0,
            activity_burst_corr: 0.3,
            flags: 0x02, // HK_CAD_HOTPLUG
            reserved1: 0,
        };
        let json = serde_json::to_value(&f).expect("serialize");
        assert_eq!(json["ceiling_violation_ratio"], 2.0);
        let back: CadenceFeatures = serde_json::from_value(json).expect("deserialize");
        assert_eq!(f, back);
    }

    /// A NaN ratio is rejected by the per-feature gate (would poison thresholds).
    #[test]
    fn nan_ratio_is_rejected() {
        let f = CadenceFeatures {
            ...
            reserved0: 0,
            hdevice_token: 0,
            declared_interval_ms: 0.0,
            observed_rate_hz: 0.0,
            ceiling_violation_ratio: f32::NAN,
            device_lifetime_s: 0.0,
            activity_burst_corr: 0.0,
            flags: 0,
            reserved1: 0,
        };
        assert!(f.ceiling_violation_ratio.is_nan());
    }
}

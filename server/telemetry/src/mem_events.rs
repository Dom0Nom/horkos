...
mod tests {
    ...
    fn region_round_trips() {
        let mut b = zeroed(SZ_REGION);
        b[0..4].copy_from_slice(&1234u32.to_le_bytes());
        b[4..8].copy_from_slice(&HK_EVENT_MEM_UNBACKED_EXEC.to_le_bytes());
        b[8..16].copy_from_slice(&0x1_0000u64.to_le_bytes());
        b[16..24].copy_from_slice(&0x2000u64.to_le_bytes());
        b[24..28].copy_from_slice(&0x4u32.to_le_bytes());
        b[28..32].copy_from_slice(&0x1u32.to_le_bytes());
        let r = MemRegion::decode(&b).expect("decode");
        assert_eq!(r.pid, 1234);
        assert_eq!(r.region_size, 0x2000);
        assert_eq!(r.flags, 0x1);
    }

    #[test]
    fn short_region_is_rejected() {
        let b = zeroed(SZ_REGION - 1);
        assert!(matches!(
            MemRegion::decode(&b),
            Err(MemEventError::Short { .. })
        ));
    }

    #[test]
    fn module_stomp_path_bounds_enforced() {
        let mut b = zeroed(SZ_MODULE_STOMP);
        // module_path_len = 209 (> PATH_CAP 208) must be rejected, not sliced.
        b[80..82].copy_from_slice(&209u16.to_le_bytes());
        assert!(matches!(
            ModuleStomp::decode(&b),
            Err(MemEventError::LenOutOfRange { cap: PATH_CAP, .. })
        ));
    }

    #[test]
    fn module_stomp_valid_path_decodes() {
        let mut b = zeroed(SZ_MODULE_STOMP);
        let name = b".text";
        b[80..82].copy_from_slice(&8u16.to_le_bytes()); // module_path_len
        b[82..84].copy_from_slice(&(name.len() as u16).to_le_bytes());
        b[84..92].copy_from_slice(b"game.exe");
        b[292..292 + name.len()].copy_from_slice(name);
        let s = ModuleStomp::decode(&b).expect("decode");
        assert_eq!(s.module_path, "game.exe");
        assert_eq!(s.section_name, ".text");
    }

    #[test]
    fn image_anomaly_oversized_len_rejected() {
        let mut b = zeroed(SZ_IMAGE_ANOMALY);
        b[16..18].copy_from_slice(&255u16.to_le_bytes()); // path_len > 208
        assert!(matches!(
            ImageAnomaly::decode(&b),
            Err(MemEventError::LenOutOfRange { .. })
        ));
    }

    #[test]
    fn unsigned_image_kernel_ships_unknown_verdict() {
        let mut b = zeroed(SZ_UNSIGNED_IMAGE);
        b[4..8].copy_from_slice(&HK_SIGN_UNKNOWN.to_le_bytes());
        b[48..50].copy_from_slice(&4u16.to_le_bytes());
        b[52..56].copy_from_slice(b"a.so");
        let u = UnsignedImage::decode(&b).expect("decode");
        assert_eq!(u.signer_verdict, HK_SIGN_UNKNOWN);
        assert_eq!(u.file_path, "a.so");
    }

    #[test]
    fn exec_origin_decodes() {
        let mut b = zeroed(SZ_EXEC_ORIGIN);
        b[4..8].copy_from_slice(&42u32.to_le_bytes());
        b[8..16].copy_from_slice(&0xdead_beefu64.to_le_bytes());
        let o = ExecOrigin::decode(&b).expect("decode");
        assert_eq!(o.thread_id, 42);
        assert_eq!(o.start_address, 0xdead_beef);
    }
}

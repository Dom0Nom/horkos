...
mod tests {
    ...
    /// A wallhack pre-aim: the aim sits exactly on an occluded, silent enemy across
    /// many engagements -> a strong (negative-tail) z-score is emitted.
    #[test]
    fn occluded_tight_lock_scores() {
        let mut a = OcclusionPreaim::new(1);
        for t in 0..MIN_ENGAGEMENTS + 2 {
            let mut snap = base_snapshot(t as u64, t as u64 * 1_000_000);
            // Enemy directly ahead (+X), occluded, no audio.
            push_enemy(
                &mut snap,
                100,
            ...
            // Aim straight ahead (zero yaw/pitch keeps forward = +X) -> ~0 error.
            let tick = TickPayload {
                aim_delta_x: 0.0,
                aim_delta_y: 0.0,
                ..Default::default()
            };
            a.feed(&tick, &snap).expect("feed");
        }
        let ev = a.score().expect("a tight occluded lock scores");
        assert_eq!(ev.signal_id, 172);
        assert!(ev.zscore > 0.0);
        assert!(ev.sample_count >= MIN_ENGAGEMENTS);
    }

    /// Clean: the enemy is VISIBLE, so even a tight lock is legitimately knowable and
    /// is never counted -> no score.
    #[test]
    fn visible_enemy_lock_is_clean() {
        let mut a = OcclusionPreaim::new(1);
        for t in 0..MIN_ENGAGEMENTS + 2 {
            let mut snap = base_snapshot(t as u64, t as u64 * 1_000_000);
            push_enemy(
        ...
        assert!(a.score().is_none(), "visible enemy -> not counted");
    }

    /// Clean: occluded but AUDIBLE enemy is legitimately trackable by sound.
    #[test]
    fn audible_occluded_enemy_is_clean() {
        let mut a = OcclusionPreaim::new(1);
        for t in 0..MIN_ENGAGEMENTS + 2 {
            let mut snap = base_snapshot(t as u64, t as u64 * 1_000_000);
            push_enemy(
            ...
            a.feed(&TickPayload::default(), &snap).expect("feed");
        }
        assert!(a.score().is_none(), "audible enemy -> not counted");
    }
}

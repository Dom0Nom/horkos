...
mod tests {
    ...
    fn encode_crossview(v: &SelfCrossView) -> Vec<u8> {
        let mut b = Vec::with_capacity(120);
        b.extend_from_slice(&v.pid.to_le_bytes());
        b.extend_from_slice(&v.section_rva.to_le_bytes());
        b.extend_from_slice(&v.image_base.to_le_bytes());
        b.extend_from_slice(&v.hash_inproc);
        b.extend_from_slice(&v.hash_kernel);
        b.extend_from_slice(&v.hash_disk);
        b.extend_from_slice(&v.match_matrix.to_le_bytes());
        b.extend_from_slice(&v.first_diff_rva.to_le_bytes());
        b
    }

    fn encode_retaddr(v: &SelfRetAddr) -> Vec<u8> {
        let mut b = Vec::with_capacity(144);
        b.extend_from_slice(&v.pid.to_le_bytes());
        b.extend_from_slice(&v.guarded_fn_id.to_le_bytes());
        for f in &v.frames {
            b.extend_from_slice(&f.to_le_bytes());
        }
        b.extend_from_slice(&v.frame_count.to_le_bytes());
        b.extend_from_slice(&v.unsigned_frame_idx.to_le_bytes());
        b.extend_from_slice(&v.shadow_stack_mismatch.to_le_bytes());
        b
    }

    #[test]
    fn crossview_round_trip_and_inline_patch_class() {
        // in-process differs from kernel==disk -> inline patch (self-read-restoring hook).
        let v = SelfCrossView {
            pid: 0x1234,
            section_rva: 0x1000,
            ...
            hash_kernel: [0xBB; 32],
            hash_disk: [0xBB; 32],
            match_matrix: HK_SELF_MATCH_KERNEL_DISK,
            first_diff_rva: 0x40,
        };
        let bytes = encode_crossview(&v);
        assert_eq!(bytes.len(), 120);
        let decoded = SelfCrossView::decode(&bytes).expect("decode");
        assert_eq!(decoded, v);
        assert_eq!(decoded.classify(), CrossViewClass::InlinePatch);
    }

    #[test]
    fn crossview_all_agree_is_clean() {
        let v = SelfCrossView {
            pid: 1,
            section_rva: 0,
            image_base: 0,
            ...
            hash_kernel: [0xCC; 32],
            hash_disk: [0xCC; 32],
            match_matrix: HK_SELF_MATCH_INPROC_KERNEL
                | HK_SELF_MATCH_KERNEL_DISK
                | HK_SELF_MATCH_INPROC_DISK,
            first_diff_rva: 0,
        };
        assert_eq!(v.classify(), CrossViewClass::AllAgree);
    }

    #[test]
    fn crossview_kernel_diverge_class() {
        // in-process==disk, kernel differs: the kernel foreign read sees another page.
        let v = SelfCrossView {
            pid: 1,
            section_rva: 0,
            image_base: 0,
            ...
            hash_kernel: [0x22; 32],
            hash_disk: [0x11; 32],
            match_matrix: HK_SELF_MATCH_INPROC_DISK,
            first_diff_rva: 8,
        };
        assert_eq!(v.classify(), CrossViewClass::KernelDiverge);
    }

    #[test]
    fn retaddr_round_trip() {
        let mut frames = [0u64; HK_SELF_MAX_FRAMES];
        frames[0] = 0x7FF6_1000;
        frames[1] = 0xDEAD_BEEF; // unsigned immediate caller
        let v = SelfRetAddr {
            pid: 9,
            guarded_fn_id: 147,
            frames,
            frame_count: 2,
            unsigned_frame_idx: 1,
            shadow_stack_mismatch: 0,
        };
        let bytes = encode_retaddr(&v);
        assert_eq!(bytes.len(), 144);
        let decoded = SelfRetAddr::decode(&bytes).expect("decode");
        assert_eq!(decoded, v);
    }

    #[test]
    fn retaddr_hostile_frame_count_is_typed_error_not_panic() {
        let mut bytes = encode_retaddr(&SelfRetAddr {
            pid: 0,
            guarded_fn_id: 0,
            frames: [0u64; HK_SELF_MAX_FRAMES],
            frame_count: 0,
            unsigned_frame_idx: HK_SELF_FRAME_NONE,
            shadow_stack_mismatch: 0,
        });
        // Stomp frame_count to a malicious 0xFFFF.
        let fc_off = 8 + HK_SELF_MAX_FRAMES * 8;
        bytes[fc_off] = 0xFF;
        bytes[fc_off + 1] = 0xFF;
        let err = SelfRetAddr::decode(&bytes).expect_err("must reject");
        match err {
            SelfEventError::OutOfRange { value, .. } => assert_eq!(value, 0xFFFF),
    ...
    fn short_buffer_is_typed_error_not_panic() {
        let short = [0u8; 10];
        let err = SelfCrossView::decode(&short).expect_err("must be Short");
        assert!(matches!(err, SelfEventError::Short { .. }));
    }

    #[test]
    fn unknown_type_is_typed_error() {
        let err = decode_event(99, &[0u8; 48]).expect_err("unknown");
        assert_eq!(err, SelfEventError::UnknownType(99));
    }

    #[test]
    fn compat_decodes_for_all_four_signals() {
        let mut b = Vec::with_capacity(48);
        b.extend_from_slice(&7u32.to_le_bytes()); // pid
        b.extend_from_slice(&152u32.to_le_bytes()); // signal_id
        b.extend_from_slice(&0u64.to_le_bytes()); // image_base
        b.extend_from_slice(&0x2000u64.to_le_bytes()); // table_rva
        b.extend_from_slice(&0x40u64.to_le_bytes()); // expected
        b.extend_from_slice(&0x40u64.to_le_bytes()); // actual
        ...
        for ty in [
            ...
            HK_EVENT_SELF_LOADER,
            HK_EVENT_SELF_WX_DRIFT,
            HK_EVENT_SELF_TLS_INIT,
        ] {
            match decode_event(ty, &b).expect("decode") {
                SelfEvent::Compat(c) => assert_eq!(c.signal_id, 152),
    ...
    #[test]
    fn fp_gate_requires_corroboration_for_high_fp_signals() {
        // 147/149/150 do NOT alert without a concurrent signature failure.
        assert!(!fp_gated_alert(HK_EVENT_SELF_RETADDR, false));
        assert!(!fp_gated_alert(HK_EVENT_SELF_IAT_TARGET, false));
        assert!(!fp_gated_alert(HK_EVENT_SELF_VEH_UNWIND, false));
        // With a concurrent signature failure they promote.
        assert!(fp_gated_alert(HK_EVENT_SELF_RETADDR, true));
        // 145/148 stand alone.
        assert!(fp_gated_alert(HK_EVENT_SELF_CROSSVIEW, false));
        assert!(fp_gated_alert(HK_EVENT_SELF_HWBP, false));
    }

    #[test]
    fn iat_forwarder_is_benign() {
        assert!(iat_is_benign(0));
        assert!(iat_is_benign(HK_SELF_TGT_FORWARDER));
        assert!(!iat_is_benign(HK_SELF_TGT_DISPLACED));
        assert!(!iat_is_benign(HK_SELF_TGT_PRIVATE | HK_SELF_TGT_UNSIGNED));
    }
}

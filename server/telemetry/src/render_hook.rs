...
mod tests {
    ...
    /// `hk_render_finding` numeric core (9 numeric fields) plus the 3 string
    /// side-channel fields. Guards against schema drift between
    /// `render_hook_schema.h` and this mirror.
    #[test]
    fn render_finding_round_trips() {
        let f = RenderFinding {
            ...
            region_hash: 0x1234,
            cadence_drift_ns: -42,
            module_path: Some("\\Device\\HarddiskVolume3\\overlay.dll".into()),
            signer_subject: Some("CN=Some Publisher".into()),
            window_class: None,
        };
        let json = serde_json::to_value(&f).expect("serialize");
        // window_class is None -> still serialized as null by default; the numeric
        // core is always present.
        assert_eq!(json["signal"], 46);
        assert_eq!(json["cadence_drift_ns"], -42);
        let back: RenderFinding = serde_json::from_value(json).expect("deserialize");
        assert_eq!(f, back);
    }

    /// String side-channel fields are optional: a finding that omits them
    /// deserializes with `None`, so a pure window finding need not carry a signer.
    #[test]
    fn string_side_channel_is_optional() {
        ...
        let f: RenderFinding = serde_json::from_value(body).expect("deserialize");
        assert_eq!(f.owning_pid, 4242);
        assert!(f.module_path.is_none());
        assert!(f.signer_subject.is_none());
        assert!(f.window_class.is_none());
    }
}

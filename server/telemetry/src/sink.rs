...
mod tests {
    ...
    fn sink_with_capacity(cap: usize) -> (TickSink, mpsc::Receiver<TickPayload>) {
        let (tx, rx) = mpsc::channel(cap);
        (TickSink::new(vec![tx]), rx)
    }

    #[test]
    fn forwards_to_shard() {
        let (sink, mut rx) = sink_with_capacity(4);
        let p = TickPayload {
            player_id: 7,
            tick: 1,
            ..Default::default()
        };
        sink.send(p.clone()).expect("send");
        assert_eq!(rx.try_recv().expect("forwarded").player_id, 7);
    }

    #[test]
    fn full_shard_is_visible_overload() {
        let (sink, _rx) = sink_with_capacity(1);
        sink.send(TickPayload::default()).expect("first fits");
        let err = sink.send(TickPayload::default()).expect_err("second sheds");
        assert!(matches!(err, TelemetryError::Overloaded));
        assert_eq!(sink.shed_full.load(Ordering::Relaxed), 1);
    }

    #[test]
    fn dead_pipeline_is_internal_error() {
        let (tx, rx) = mpsc::channel::<TickPayload>(1);
        drop(rx);
        let sink = TickSink::new(vec![tx]);
        let err = sink.send(TickPayload::default()).expect_err("closed");
        assert!(matches!(err, TelemetryError::Internal));
    }

    #[test]
    fn token_bucket_sheds_flood_per_player() {
        // Capacity well above burst so the bucket (not the channel) is the gate.
        let (sink, _rx) = sink_with_capacity(2048);
        let mut limited = 0u32;
        for i in 0..(BUCKET_BURST as u64 + 64) {
            let p = TickPayload {
                player_id: 1,
                tick: i,
                ..Default::default()
            };
            if matches!(sink.send(p), Err(TelemetryError::RateLimited)) {
                limited += 1;
            }
        }
        assert!(limited > 0, "flood past burst is rate limited");
        // A different player is unaffected by player 1's flood.
        let other = TickPayload {
            player_id: 2,
            ..Default::default()
        };
        sink.send(other).expect("other player admitted");
    }
}

...
mod tests {
    ...
    #[test]
    fn proton_override_round_trip_and_off_manifest() {
        let e = ProtonOverride {
            pid: 4242,
            flags: HK_PROTON_NATIVE_SHADOWS_BUILTIN | HK_PROTON_OFF_MANIFEST,
            override_token_hash: 0xDEAD_BEEF_CAFE_F00D,
            proton_build_hash: 0x0102_0304_0506_0708,
        };
        let mut b = Vec::new();
        enc_u32(&mut b, e.pid);
        enc_u32(&mut b, e.flags);
        enc_u64(&mut b, e.override_token_hash);
        enc_u64(&mut b, e.proton_build_hash);
        assert_eq!(b.len(), 24);
        let d = ProtonOverride::decode(&b).expect("decode");
        assert_eq!(d, e);
        assert!(d.features().override_off_manifest);
    }

    #[test]
    fn dxvk_native_override_does_not_flag() {
        // A legitimate DXVK d3d11=n override: the verifier resolved it as on the FP
        // allowlist and not shadowing a builtin-only DLL, so neither bit is set.
        let e = ProtonOverride {
            pid: 4242,
            flags: 0,
            override_token_hash: 0x1111,
            proton_build_hash: 0x2222,
        };
        assert!(!e.features().override_off_manifest);
    }

    #[test]
    fn foreign_map_memfd_round_trip() {
        let e = ForeignMap {
            pid: 9,
            prot_flags: 0x4, // PROT_EXEC
            map_base: 0x7F00_0000_0000,
            backing_inode: 0,
            backing_dev: 0,
            map_flags: HK_MAP_MEMFD,
        };
        let mut b = Vec::new();
        enc_u32(&mut b, e.pid);
        enc_u32(&mut b, e.prot_flags);
        enc_u64(&mut b, e.map_base);
        enc_u64(&mut b, e.backing_inode);
        enc_u32(&mut b, e.backing_dev);
        enc_u32(&mut b, e.map_flags);
        assert_eq!(b.len(), 32);
        let d = ForeignMap::decode(&b).expect("decode");
        assert_eq!(d, e);
        assert!(d.features().foreign_map_off_tree);
    }

    #[test]
    fn overlay_so_on_allowlist_does_not_flag() {
        // MangoHud overlay: verifier cleared OFF_TREE/MEMFD.
        let e = ForeignMap {
            pid: 9,
            prot_flags: 0x4,
            map_base: 0x5000,
        ...
        assert!(!e.features().foreign_map_off_tree);
    }

    #[test]
    fn cross_mem_external_scanner_and_wineserver() {
        let scanner = CrossMem {
            caller_tgid: 1000,
            target_tgid: 2000,
            access_kind: HK_XMEM_PROCMEM,
            flags: 0,
            remote_addr: 0x4000_0000,
            remote_len: 0x1000,
            event_time_ns: 42,
        };
        let mut b = Vec::new();
        enc_u32(&mut b, scanner.caller_tgid);
        enc_u32(&mut b, scanner.target_tgid);
        enc_u32(&mut b, scanner.access_kind);
        enc_u32(&mut b, scanner.flags);
        enc_u64(&mut b, scanner.remote_addr);
        enc_u64(&mut b, scanner.remote_len);
        enc_u64(&mut b, scanner.event_time_ns);
        assert_eq!(b.len(), 40);
        let d = CrossMem::decode(&b).expect("decode");
        assert_eq!(d, scanner);
        assert!(d.features().cross_mem_non_wineserver);

        // The game's own wineserver read is flagged WINESERVER (reported, not dropped),
        // and must NOT extract as a non-wineserver scanner.
        let mut ws = scanner;
        ws.flags = HK_XMEM_FLAG_WINESERVER;
        assert!(!ws.features().cross_mem_non_wineserver);
    }

    #[test]
    fn ns_entry_off_lineage() {
        let e = NsEntry {
            caller_tgid: 7,
            target_ns_type: HK_NS_MNT,
            target_ns_inode: 0xF00D,
            game_ns_inode: 0xF00D,
            flags: HK_NS_FLAG_OFF_LINEAGE,
            reserved: 0,
        };
        let mut b = Vec::new();
        enc_u32(&mut b, e.caller_tgid);
        enc_u32(&mut b, e.target_ns_type);
        enc_u64(&mut b, e.target_ns_inode);
        enc_u64(&mut b, e.game_ns_inode);
        enc_u32(&mut b, e.flags);
        enc_u32(&mut b, e.reserved);
        assert_eq!(b.len(), 32);
        let d = NsEntry::decode(&b).expect("decode");
        assert_eq!(d, e);
        assert!(d.features().ns_off_lineage);
    }

    #[test]
    fn module_post_boot_vs_hotplug() {
        let cheat = ModuleLoad {
            initiator_tgid: 3,
            flags: HK_MOD_POST_BOOT | HK_MOD_OFF_BASELINE,
            module_name_hash: 0xAB,
            module_sig_hash: 0,
            event_time_ns: 1,
        };
        let mut b = Vec::new();
        enc_u32(&mut b, cheat.initiator_tgid);
        enc_u32(&mut b, cheat.flags);
        enc_u64(&mut b, cheat.module_name_hash);
        enc_u64(&mut b, cheat.module_sig_hash);
        enc_u64(&mut b, cheat.event_time_ns);
        assert_eq!(b.len(), 32);
        let d = ModuleLoad::decode(&b).expect("decode");
        assert_eq!(d, cheat);
        assert!(d.features().module_post_boot);

        // A post-boot hotplug (xpad) is flagged distinctly, not as a cheat.
        let mut hotplug = cheat;
        hotplug.flags = HK_MOD_POST_BOOT | HK_MOD_HOTPLUG;
        assert!(!hotplug.features().module_post_boot);
    }

    #[test]
    fn rootfs_rw_outside_vs_update_window_vs_desktop() {
        let breach = RootfsRw {
            actor_tgid: 5,
            flags: HK_ROOTFS_IMMUTABLE_DISTRO | HK_ROOTFS_REMOUNT_RW | HK_ROOTFS_PROTECTED_WRITE,
            target_path_hash: 0x99,
            event_time_ns: 7,
        };
        let mut b = Vec::new();
        enc_u32(&mut b, breach.actor_tgid);
        enc_u32(&mut b, breach.flags);
        enc_u64(&mut b, breach.target_path_hash);
        enc_u64(&mut b, breach.event_time_ns);
        assert_eq!(b.len(), 24);
        let d = RootfsRw::decode(&b).expect("decode");
        assert_eq!(d, breach);
        assert!(d.features().rootfs_rw_outside_window);

        // An update-window RW transition (frzr/rauc) is tagged and NOT a breach.
        let mut upd = breach;
        upd.flags |= HK_ROOTFS_UPDATE_WINDOW;
        assert!(!upd.features().rootfs_rw_outside_window);

        // A desktop distro never sets IMMUTABLE_DISTRO -> no breach.
        let mut desktop = breach;
        desktop.flags = HK_ROOTFS_REMOUNT_RW;
        assert!(!desktop.features().rootfs_rw_outside_window);
    }

    #[test]
    fn frame_consumer_is_low_weight_corroborator() {
        let e = FrameConsumer {
            consumer_tgid: 11,
            flags: HK_FRAME_WAYLAND | HK_FRAME_OFF_ALLOWLIST,
            socket_or_fb_hash: 0x33,
            event_time_ns: 4,
        };
        let mut b = Vec::new();
        enc_u32(&mut b, e.consumer_tgid);
        enc_u32(&mut b, e.flags);
        enc_u64(&mut b, e.socket_or_fb_hash);
        enc_u64(&mut b, e.event_time_ns);
        assert_eq!(b.len(), 24);
        let d = FrameConsumer::decode(&b).expect("decode");
        assert_eq!(d, e);
        let f = d.features();
        assert!(f.frame_off_allowlist);
        assert!(f.corroborator_only); // never a standalone ban input
    }

    #[test]
    fn wx_arm_builtin_stomp_vs_relocation() {
        let stomp = WxArm {
            pid: 13,
            new_prot: 0x5, // PROT_READ|PROT_EXEC re-arm
            vma_start: 0x1000,
            vma_end: 0x2000,
            backing_inode: 555,
            backing_dev: 8,
            flags: HK_WX_WAS_RX | HK_WX_IN_BUILTIN | HK_WX_INODE_OFF_MANIFEST,
        };
        let mut b = Vec::new();
        enc_u32(&mut b, stomp.pid);
        enc_u32(&mut b, stomp.new_prot);
        enc_u64(&mut b, stomp.vma_start);
        enc_u64(&mut b, stomp.vma_end);
        enc_u64(&mut b, stomp.backing_inode);
        enc_u32(&mut b, stomp.backing_dev);
        enc_u32(&mut b, stomp.flags);
        assert_eq!(b.len(), 40);
        let d = WxArm::decode(&b).expect("decode");
        assert_eq!(d, stomp);
        assert!(d.features().builtin_inode_mismatch);

        // A legitimate relocated builtin matches on-disk -> INODE_OFF_MANIFEST clear.
        let mut reloc = stomp;
        reloc.flags = HK_WX_WAS_RX | HK_WX_IN_BUILTIN;
        assert!(!reloc.features().builtin_inode_mismatch);
    }

    #[test]
    fn synth_input_mid_session_vs_steam_input() {
        let macro_dev = SynthInput {
            injector_tgid: 17,
            flags: HK_SYNTH_UINPUT_CREATE | HK_SYNTH_OFF_ALLOWLIST | HK_SYNTH_MID_SESSION,
            ev_type: 1, // EV_KEY
            ev_code: 30,
            event_time_ns: 99,
        };
        let mut b = Vec::new();
        enc_u32(&mut b, macro_dev.injector_tgid);
        enc_u32(&mut b, macro_dev.flags);
        enc_u32(&mut b, macro_dev.ev_type);
        enc_u32(&mut b, macro_dev.ev_code);
        enc_u64(&mut b, macro_dev.event_time_ns);
        assert_eq!(b.len(), 24);
        let d = SynthInput::decode(&b).expect("decode");
        assert_eq!(d, macro_dev);
        let f = d.features();
        assert!(f.synth_input_mid_session);
        assert!(f.corroborator_only);

        // Steam Input's own pre-focus allowlisted device: verifier cleared OFF_ALLOWLIST.
        let mut steam = macro_dev;
        steam.flags = HK_SYNTH_UINPUT_CREATE;
        assert!(!steam.features().synth_input_mid_session);
    }

    #[test]
    fn short_buffer_is_typed_error_not_panic() {
        let short = [0u8; 10];
        let err = CrossMem::decode(&short).expect_err("must be Short");
        match err {
            LinuxProtonError::Short { got, .. } => assert_eq!(got, 10),
    ...
    fn unknown_type_is_typed_error() {
        let err = decode_event(99, &[0u8; 40]).expect_err("unknown");
        assert_eq!(err, LinuxProtonError::UnknownType(99));
    }
    ...
    fn decode_event_dispatches_by_type() {
        let mut b = Vec::new();
        enc_u32(&mut b, 1); // pid
        enc_u32(&mut b, 0); // flags
        enc_u64(&mut b, 0); // override_token_hash
        enc_u64(&mut b, 0); // proton_build_hash
        match decode_event(HK_EVENT_PROTON_OVERRIDE, &b).expect("decode") {
            LinuxProtonEvent::ProtonOverride(d) => assert_eq!(d.pid, 1),
